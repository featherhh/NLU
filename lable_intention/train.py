...
from Medical_intention.utils.data_loader import *
...
def model2train():
    ...
    my_optim = optim.AdamW(my_model.parameters(), lr=conf.lr, weight_decay=1e-5)
    # 实例化损失函数对象
    ...
    for epoch_idx in range(conf.epochs):
        ...
        torch.save(my_model.state_dict(), r'F:\ai\第八阶段-nlp2\完整代码\MedicalKB\NLU\Medical_intention\save_model/epoch_%s.pth'%(epoch_idx+1))
    end_time = time.time()
